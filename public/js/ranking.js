    ...
    try {
      ...
      const badgesHtml = badges.length > 0
        ? '<div style="font-size:11px;color:#a0aec0;font-weight:700;letter-spacing:1px;margin-bottom:10px">SUAS CONQUISTAS</div>'
          ...
          + badges.map(b => {
              const emoji = b.descricao.split(' ')[0];
              const nome  = b.descricao.substring(b.descricao.indexOf(' ')+1);
      ...
      const pontosHtml = [
        ...
        ['🏁 Checkout','+ 5'],['🔥 Streak bônus (a cada 5 dias)','+ 25'],
        ['❌ Cancelamento','- 20'],['📵 Checkout sem GPS','- 5']
      ].map(function(row) {
      ...
      el.innerHTML =
        ...
        + '<div style="background:rgba(246,173,85,0.1);border-radius:10px;padding:12px"><div style="font-size:28px;font-weight:800;color:#f6ad55">' + scoreTotal + '</div><div style="font-size:10px;color:#a0aec0;margin-top:2px">TOTAL</div></div>'
        + '<div style="background:rgba(104,211,145,0.1);border-radius:10px;padding:12px"><div style="font-size:28px;font-weight:800;color:#68d391">+' + ganhoSemana + '</div><div style="font-size:10px;color:#a0aec0;margin-top:2px">SEMANA</div></div>'
        + '<div style="background:rgba(99,179,237,0.1);border-radius:10px;padding:12px"><div style="font-size:28px;font-weight:800;color:#63b3ed">' + minhaPos + 'º</div><div style="font-size:10px;color:#a0aec0;margin-top:2px">POSIÇÃO</div></div>'
        + '</div>'
        + (meuStreak >= 3 ? '<div style="text-align:center"><div style="display:inline-flex;align-items:center;gap:6px;background:rgba(246,173,85,0.15);border:1px solid rgba(246,173,85,0.3);border-radius:20px;padding:4px 14px"><span style="font-size:16px">🔥</span><span style="font-size:13px;font-weight:700;color:#f6ad55">' + meuStreak + ' dias consecutivos</span></div></div>' : '')
        + '</div>'
        + badgesHtml
